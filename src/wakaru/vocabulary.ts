...
export interface VocabularyAssistant {
...
export class JapaneseVocabulary {
  ...
  public async analyse(
    ...
    if (!candidates.length) {
      const generated = await this.assistant.define(expression, context);
      return {
        tokens,
    ...
    if (context && this.assistant.availability !== 'unavailable') {
      try {
        const rankedIds = await this.assistant.rank(
          expression,
          context,
          candidates
        );
  ...
  public async prepare(
    ...
    return this.assistant.addExample(candidate, context);
  }
}
