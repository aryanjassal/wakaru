...
import { configDir } from '@/wakaru/config.js';
import { resolveUserPath } from '@/wakaru/utils.js';
import { WakaruAssetNotFoundError } from '@/wakaru/errors.js';
...
function resolveAssetPath(override: string | undefined, name: string): string {
  ...
  const workingTree = join(process.cwd(), 'assets/runtime', name);
